...
DROP FUNCTION IF EXISTS public.send_message_to_conversation(UUID, UUID, TEXT, TEXT[], UUID, BOOLEAN, UUID);
DROP FUNCTION IF EXISTS public.send_message_to_conversation(UUID, UUID, TEXT, UUID);

-- Create a single unified version of the function
...
BEGIN
  ...
  -- Update conversation to trigger the timestamp update
  UPDATE conversations
  SET id = id  -- This will trigger the update_timestamp trigger
  WHERE id = conv_id;
